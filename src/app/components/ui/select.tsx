...
>(({ className, children, ...props }, ref) => (
  ...
  >
    {children}
    <span className="ml-2 text-mutedForeground">▾</span>
  </SelectPrimitive.Trigger>
));
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "z-50 min-w-32 overflow-hidden rounded-card border border-border bg-popover text-popoverForeground shadow-sm",
        className
      )}
      position={position}
      {...props}
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded px-2 py-1.5 text-sm outline-none focus:bg-muted data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className
    )}
    {...props}
  >
