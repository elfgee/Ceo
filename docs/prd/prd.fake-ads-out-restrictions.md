...
- [ ] 제한 조치가 없는 경우, 컨텐츠 헤더의 sub text는 미노출
...
**레이아웃**:
- [ ] 해당 정보는 페이지 컨텐츠 헤더(`ContentAreaHeader`)의 **sub text 영역**에 표시
- [ ] 별도의 섹션(카드) 제목을 추가로 만들지 않음 (페이지 타이틀과 중복 방지)
...
**표시 형식**:
- [ ] "내가 받은 제한 조치: {처리} | {위반 정책} | {날짜}"

**시각적 규칙**:
- [ ] 제한 조치가 있는 경우, sub text는 destructive 컬러로 강조 표시
